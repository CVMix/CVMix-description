...
bottom do not exist in MOM or POP.  We also motivate simplifications
to the scheme relative to the original implementation based on
\cite{LargeKPP}.
...
The second term, $\gamma_{\lambda}$, accounts
for non-local transport that is not directly associated with local
...
\end{equation}
\cite{LargeKPP} proposed to set the diffusivity, $K^{\mbox{\tiny
    non-local}}_{\lambda}$, equal to the diffusivity $K_{\lambda}$.
However, there are cases where the two diffusivities take on distinct
matching conditions at the boundary layer base, in which case they are
different.  We have more to say on this topic in Section
...
\end{equation}
where the diffusivity computed as part of the local flux is given by
...
is generally the same as $K_{\lambda}$, with some exceptions to be
...
(\ref{eq:specifying-shape-function}), we are led to an expression
...
boundary layer diffusivity to the interior diffusivity.  We question
this approach in Section \ref{subsection:kpp-shape-function-cvmix}, in
which we argue for the use a the universal shape function given by
equation (\ref{eq:universal-non-local-structure}).
...
The discussion of the shape function $G_{\lambda}$ in Section
...
\item The MOM code takes the buoyancy frequency $N$ appearing in
