...
\begin{mdframed}[backgroundcolor=lightgray!50]
\begin{align}
\overline{w \, \lambda}^{\mbox{\tiny non-local}} &= 0  \qquad \mbox{if} \; \; B_{f} > 0
\\
\overline{w \, \theta}^{\mbox{\tiny non-local}} &= K_{\theta}  \, \gamma_{\theta}
  = 
...
$G_{\theta}(\sigma) \, C_{s}$ and $G_{s}(\sigma) \, C_{s}$, and returns
them to the calling model, which then computes the non-local flux upon
multiplication by the boundary flux.


\subsubsection{Non-local transport problem I: non-local flux larger than surface flux}

We now identify some problems that have been observed related to the
non-local transport term.  These problems remain under investigation,
and there is no consensus amongst the developers how to proceed.
Various options are listed here to address the problems.
...
\subsubsection{Non-local transport problem II: spuriously large divergence}
...
exacerbated in the following situations of growing importance for
regional and global models using the KPP scheme.
\begin{itemize}
...
cubic function for {\it all} tracers
\begin{equation}
G(\sigma)_{\mbox{\tiny non-local}}= \sigma \, (1-\sigma)^{2}   \qquad \mbox{for} \; \; \sigma \le 1. 
\label{eq:universal-non-local-structure}
\end{equation}
As noted in Figure \ref{fig:kpp-figure2-reproduced}, this structure
function arises from matching to a zero interior value of the
diffusivity, along with zero slope at the boundary layer base.  This
structure function removes problems with the spuriously large
divergences across the boundary layer base since the non-local term
smoothly goes to zero at $\sigma =1$.  Additionally, the maximum value
for the product $G(\sigma)_{\mbox{\tiny non-local}} \, C_{s}$ is
\begin{equation}
...
(\ref{eq:Cs-value})).  Hence, the structure function
$G(\sigma)_{\mbox{\tiny non-local}}$ removes problems with the
non-local flux being larger than the surface flux.
