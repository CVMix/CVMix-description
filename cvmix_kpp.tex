...
In particular, we simplify the specification of \cite{LargeKPP}.  But
we note here that the polynomial coefficients $a_{1}, a_{2}, a_{3}$
depend on the tracer fields in so far as they are specified by
matching to the interior diffusivities.  Double diffusive processes
lead to distinct diffusivities for temperature and material tracers
such as salt.
...
This is a reproduction of
 ...
 \hspace{0.4cm} The horizontal axis on the bottom is the mean buoyancy, $B$,
  ...
  \overline{w \, b}^{0}$, where $\beta_{T} = -0.2$. This situation
...
Appendix C of][]{CORE_NYF}. For our purposes, we assume $\bftau$ is
given, thus yielding the ocean kinematic fluxes associated with the
turbulent transport of momentum across the ocean surface at $d = -z +
\eta = 0$
\begin{equation}
 -\overline{w \, {\bf u}}^{0} 
...
\begin{align}
  \overline{w \, \lambda} &= -K_{\lambda} \left( \frac{\partial \Lambda}{\partial z} - \gamma_{\lambda} \right)
 \\
 &= -K_{\lambda} \left( \frac{\partial \Lambda}{\partial z} \right)   +   K_{\lambda} \,  \gamma_{\lambda}, 
...
forcing, and depth from the ocean surface.
...
surface value, $\overline{w \, \lambda}^{0}$.  We may thus use this
...
\begin{equation}
 G(\sigma) = \sigma \, (1-\sigma)^{2}   \qquad \mbox{for} \; \; G(\sigma=0) = G(\sigma=1) = G'(\sigma=1) = 0, 
\label{eq:simple-structure-function}
\end{equation}
...
\end{equation}
The resulting form for $G(\sigma)$ is given by 
\begin{equation}
  G(\sigma) = \sigma \, (1-\sigma)^{2}+ \sigma^{2} \,  (3 - 2 \, \sigma) \, \left(\frac{K_{\lambda}(h)}{h \, w_{\lambda}(h)} \right)
  \qquad \mbox{for} \; \; G(\sigma=0) = G'(\sigma=1) = 0. 
\end{equation}
Notice how the structure function reduces to the simpler form
$G(\sigma) = \sigma \, (1-\sigma)^{2}$ when matching to a zero
diffusivity, $K_{\lambda}(h) = 0$, at the boundary layer base (see
equation (\ref{eq:simple-structure-function})).
...
\begin{align}
 \gamma_{\theta} &= 
   C_{s} \, \left( \frac{ \overline{w \, \theta}^{0} - Q_{R}/(\rho_{0} \, C_{\mbox{\tiny p}}^{o})  }{h \,  w_{\theta}(\sigma)}  
            \right) 
...
\overline{w \, \theta}^{\mbox{\tiny non-local}} &= K_{\theta}  \, \gamma_{\theta}
  = 
 G_{\theta}(\sigma) \, C_{s} \, \left( \overline{w \, \theta}^{0} - Q_{R}/(\rho_{0} \, C_{\mbox{\tiny p}}^{o}) \right)
...
The above discussion of the non-local transport follows the general
prescription from \cite{LargeKPP}.  Unfortunately, there are problems
related to the non-local transport term.  These problems remain under
investigation, with various options listed here to address the
problems.

The first problem relates to cases where the parameteried non-local
flux, (\ref{eq:non-local-flux-kpp-param-temp}) of
...
value for the shape function $G(\sigma)$.  In this case, one may be
exposed to the production of extrema in the tracer field.  In the
presence of sea-ice, problems may arise particularly in fresh water
regions such as the Baltic Sea where the thermal expansion coefficient
is negative, $\alpha < 0$ (Martin Schmidt, personal communication).
...
\item {\sc interior gravitational instabilities}: When the vertical
  ...
  become quite large in magnitude, thus leading to $
...
vanish beneath the boundary layer.  Hence, by determining the
structure function $G_{\lambda}(\sigma)$ according to the needs of
diffusivity matching, then the non-local transport will in turn have a
jump at the boundary layer base.  Only for those cases where the
interior diffusivity vanishes will both the diffusivity and the
non-local transport have a smooth transition from the boundary layer
into the interior.
...
In essence, the fundamental problem identified here is that the
non-local transport parameterization proposed by \cite{LargeKPP} does
not smoothly approach zero at the boundary layer base in those cases
where the interior diffusivity is nonzero.  The problem is present in
all known implementations of the KPP non-local transport.  However,
its adverse effects are generally mild for models with either small
interior diffusivities or with coarse vertical grid spacing.  Modern
global models, and many regional models, are pushing the limits where
interior mixing is sizable and vertical grid spacing refined. 

One solution to this problem is to use two distinct structure
functions: one for the diffusivity and another for the non-local
transport.  The diffusivity structure function, $G_{\lambda}(\sigma)$
will remain as originally proposed by \cite{LargeKPP}, with matching
to the interior diffusivity along with the caveats noted above.
However, for the non-local transport, we propose to use the universal
cubic function (\ref{eq:simple-structure-function}) for {\it all}
tracers
\begin{equation}
G(\sigma)_{\mbox{\tiny non-local}}= \sigma \, (1-\sigma)^{2}   \qquad \mbox{for} \; \; \sigma \le 1. 
\label{eq:universal-non-local-structure}
\end{equation}
As noted in the discussion of equation
(\ref{eq:simple-structure-function}) (see also Figure
\ref{fig:kpp-figure2-reproduced}), this structure function arises from
matching the function and its derivative to zero at the boundary layer
base.  This structure function removes problems with the spuriously
large divergences across the boundary layer base since the non-local
term smoothly goes to zero at $\sigma =1$.  Additionally, the
structure function $G(\sigma)_{\mbox{\tiny non-local}}$ has a maximum
at $\sigma = 1/3$, where $G'(\sigma)_{\mbox{\tiny non-local}} = 1
-4\sigma + 3\sigma^2 = 0$.  Hence, the maximum value for the product
$G(\sigma)_{\mbox{\tiny non-local}} \, C_{s}$ is
\begin{equation}
 \left[ G(\sigma=1/3)_{\mbox{\tiny non-local}} \, C_{s} \right]^{\mbox{\tiny maximum}} =  \left( \frac{4}{27}  \right) \, 6.32 \approx 0.94 < 1, 
\end{equation}
using $C_{s} = 6.32$ suggested by \cite{LargeKPP} (see equation
(\ref{eq:Cs-value})).  Since $\left[ G(\sigma=1/3)_{\mbox{\tiny
      non-local}} \, C_{s} \right]^{\mbox{\tiny maximum}} < 1$, the
structure function $G(\sigma)_{\mbox{\tiny non-local}}$ removes
...
C_{v} =  \left\{ 
\begin{array}{lc}
  2.1 - 200 \, N   & \qquad N \le 0.002 \, \mbox{sec}^{-1}
\\
  1.7 & \mbox{otherwise}. 
\end{array}
\right.
...
\item The constant $c_{s}$ is part of the similarity function for
  ...
  \ref{subsection:m-o-similarity-functions}.
...
\item The MOM and POP codes take the buoyancy frequency $N$ appearing
  in equations (\ref{eq:unresolved-shear-kpp}) and
  (\ref{eq:Cv-values-given}) to be the absolute value of the frequency
\begin{equation}
 N  = |N|.  
\end{equation}
This choice seems problematic in regions of unstable buoyancy forcing,
where $N^{2} < 0$ is common.\footnote{\color{red} SMG: follow up with
  Gokhan on this issue. How to think about it?}
...
\item[$\bullet$] $\beta_{T} = \overline{w \, b}^{d=h_{e}} /
  \overline{w \, b}^{0}$ = ratio of the turbulent buoyancy flux at a
...
\item[$\bullet$] $S_{*} = -\overline{w \, s}^{0} / u_{*}$ = scale
  ...
  turbulent fluxes leading to surface ocean freshening, $\overline{w
    \, s}^{0} > 0$, correspond to a negative turbulent salinity
  scale, $S_{*} < 0$.
