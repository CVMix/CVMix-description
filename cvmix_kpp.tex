...
(equation (\ref (\ref{eq:non-local-shape-function-parabolic})).
This small sensitivity contrasts to the results from the original
shape function as shown in Figure \ref{fig:KPP_temperature_test}.
All three of the new shape functions show similar small
sensitivity to the vertical grid.  Furthermore, there is no sign of
spurious warming with the new functions.  
...
\item[$\bullet$] $\mathrm{d}z$ = thickness (metre) of a tracer cell
 ...
 \item[$\bullet$] $\mathcal{G}(\sigma)$ = non-dimensional vertical
   ...
   \ref{eq:non-local-shape-function})).  This structure function
   is distinct from that used for the diffusivity.
